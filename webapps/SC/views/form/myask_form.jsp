	...
	<c:choose>
		<c:when test="${requestScope.isupdate}">
			<form method="post" action="Board.do" name="frm">
				<input type="hidden" name="board_mode" value="ask_update">
				<input type="hidden" name="num" value="${requestScope.board.getNum()}">
		</c:when>
		<c:otherwise>
			<form method="post" action="Board.do" name="frm">
				<input type="hidden" name="board_mode" value="ask_insert">
		</c:otherwise>
	</c:choose>
		<div id="board_insert_wrap">
			...
			<div id="button_box">
				<c:choose>
					<c:when test="${requestScope.board == null}">
						<input type="submit" id="submit_button" value="작 성" onclick="return board_alert('ask')">
					</c:when>
					<c:otherwise>
						<input type="submit" id="submit_button" value="수 정" onclick="return board_alert('ask')">
					</c:otherwise>
				</c:choose>	
			</div>
		</div>
